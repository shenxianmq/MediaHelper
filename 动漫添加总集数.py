...
if mkv_list:
    mkv_list.sort(key=lambda x:int(int(re.findall('S0*(\d+)',x)[0])*1000+int(re.findall('第0*(\d+)集',x)[0])))
if nfo_list:
    nfo_list.sort(key=lambda x:int(int(re.findall('S0*(\d+)',x)[0])*1000+int(re.findall('第0*(\d+)集',x)[0])))
if ass_list:
    ass_list.sort(key=lambda x:int(int(re.findall('S0*(\d+)',x)[0])*1000+int(re.findall('第0*(\d+)集',x)[0])))
