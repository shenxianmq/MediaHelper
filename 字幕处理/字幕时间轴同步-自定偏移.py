...
#本程序通过alass获取声音开头的正确时间，然后再把对应的字幕文件的时间进行整体偏移，只需要用alass一次
...
class VideoSubtitleProcessor:
    ...
    def create_widgets(self):
        ...
        frame_options = ttk.LabelFrame(self.root, text="同步选项")
        frame_options.pack(padx=10, pady=5, fill=tk.BOTH, expand=True)

        tk.Label(frame_options, text="时间偏移：").grid(row=0, column=0, padx=5, pady=5)
        tk.Entry(frame_options, textvariable=self.offset, width=10).grid(row=0, column=1, padx=5, pady=5)
        self.offset.set("0")
        button_frame = tk.Frame(self.root)
        button_frame.pack(padx=10, pady=5)
        tk.Button(button_frame, text="获取偏移", command=self.start_processing).pack(side=tk.LEFT, padx=10,pady=10)
        tk.Button(button_frame, text="批量处理", command=self.start_to_offset).pack(side=tk.LEFT, padx=10,pady=10)
    ...
    def start_to_offset(self):
        if not self.folder_path.get():
            self.update_status('请先选择一个文件夹。')
            return

        self.process_thread = threading.Thread(target=self.sub_offset)
        self.process_thread.start()

    def sub_offset(self):
        count = 0
        self.progress_text.set("正在同步字幕时间轴...")
        self.output_text.delete(1.0, tk.END)
        time_delay = int(self.offset.get())
        for root, _, files in os.walk(self.folder_path.get()):
            for file in files:
                ...
                if file_path.endswith('.ass') :
                    ...
                    self.output_text.insert(tk.END,f'字幕时间偏移同步成功:{file_path}，已同步{count}个字幕\n')
                elif file_path.endswith('.srt'):
                    count += 1
                    self.shift_subtitle_timeline_srt(file_path,time_delay)
                    self.output_text.insert(tk.END,f'字幕时间偏移同步成功:{file_path}，已同步{count}个字幕\n')
        self.progress_text.set(f"同步成功:共同步{count}个字幕。")
                    ...
                    for subtitle_path in subtitle_path_list:
                        if os.path.exists(subtitle_path):
                            ...
                            if current_os == "Windows":
                                cmd = [r"C:\alass\alass.bat", video_filepath, subtitle_path, new_subtitle_path,"-l"]
                                subprocess.call(cmd)
                            elif current_os == "Darwin":
                                working_directory = "/Users/shenxian/alass"
