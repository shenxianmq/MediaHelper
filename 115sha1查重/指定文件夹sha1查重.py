...
class DuplicateFileFinder:
    ...
    def find_duplicate_files(self):
        hash_map = {}
        duplicate_files = []
        fs_dir_path = self.root_path.replace(self.clouddrive2_root_path, "")
            ...
            for filename in filenames:
                ...
                if file_extension in [
                    ...
                    if file_sha1 in hash_map:
                        existing_file = hash_map[file_sha1]
                        duplicate_files.append((hash_map[file_sha1], file_path))
                        if len(existing_file) < len(file_path):
                            try:
                                self.fs.remove(existing_file)
                            except:
                                pass
                            print(f"已经删除sha1重复的文件::: {existing_file}")
...
if __name__ == "__main__":
    ...
    duplicate_files = finder.find_duplicate_files()
