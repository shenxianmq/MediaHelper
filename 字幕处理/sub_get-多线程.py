...
def get_sub_info(mkv_path):
    ffmpeg_command = f'ffprobe -v error -select_streams s -show_entries stream=index:stream_tags=language:format_tags=title -of default=noprint_wrappers=1 "{mkv_path}"'
...
class SubtitleExtractor:
    ...
    def create_widgets(self):
        ...
        frame_options = ttk.LabelFrame(self.root, text="提取选项")
        frame_options.pack(padx=10, pady=5, fill=tk.BOTH, expand=True)

        tk.Label(frame_options, text="字幕流编号：").grid(row=0, column=0, padx=5, pady=5)
        self.sub_num_combo = ttk.Combobox(frame_options, textvariable=self.sub_num, state="readonly")
        self.sub_num_combo['values'] = list(range(1, 40))  # 设置下拉菜单范围为1到39
        self.sub_num_combo.grid(row=0, column=1, padx=5, pady=5)
        self.sub_num_combo.set('1')  # 设置默认选择项


        tk.Label(frame_options, text="字幕格式：").grid(row=0, column=2, padx=5, pady=5)
        # tk.Entry(frame_options, textvariable=self.sub_ext, width=10).grid(row=0, column=3, padx=5, pady=5)
        tk.OptionMenu(frame_options, self.sub_ext, *self.font_types).grid(row=0, column=3, padx=5, pady=5)
        self.sub_ext.set("srt")
        tk.Checkbutton(frame_options, text="自动识别", variable=self.auto_process).grid(row=0, column=4, padx=5, pady=5)
        self.auto_process.set(True)
        ...
        try:
            ...
            for line in lines:
                if re.search(r'Stream #0:\d+\(.*?\): Subtitle: .*', line):
                    ...
                    if title_match and subtitle_info:
                        subtitle_info[-1][2] = title_match.group(1).strip()
            self.insert_message('字幕信息:\n')
            for index,sub_type,title,subtitle_lang in [item for item in subtitle_info if item[2]]:
                sub_info = f"{int(index.strip())-1}:{title.strip()}.{sub_type}\n"
                self.insert_message(sub_info)
            chi_sub = [item for item in subtitle_info if "chi" in item[3] or "Chi" in item[3] or 'zh' in item[3]]
            chi_sub = [item for item in chi_sub if "简" in item[2] or "Simplified" in item[2] or "chs" in item[2]]
            chi_sub_res = [('1','srt','chi')]
            if chi_sub:
                self.insert_message(f'找到中文字幕:\n')
                chi_sub_res = chi_sub[:]
            sub_num = str(int(chi_sub_res[0][0])-1)
            sub_ext = chi_sub_res[0][1]
            sub_name = chi_sub_res[0][2]
            self.insert_message(f'序号:{sub_num},名称:{sub_name},类型:{sub_ext}\n')
            self.sub_num.set(sub_num)
            self.sub_ext.set(sub_ext)
            self.progress_text.set(f"字幕信息获取成功")
            return int(sub_num)-1,sub_ext
        except Exception as e:
            self.insert_message(f"发生错误：{str(e)}\n")
        return
        ...
        for root, _, files in os.walk(folder_path):
            for file in files:
                if file.endswith('.mkv') or file.endswith('.mp4'):
                    ...
                    self.insert_message(f"成功提取字幕: {file}\n")
